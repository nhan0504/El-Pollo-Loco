...
describe('Poll Endpoints', () => {
  ...
  it('should create a new poll', async () => {
    ...
    const response = await agent.post('/polls').send(newPoll).expect(201);
  ...
  it('should get a poll by id', async () => {
    const response = await agent.get(`/polls/${createdPollId}`).expect(200);
  ...
  it('should delete the poll', async () => {
    await agent.delete(`/polls/${createdPollId}`).expect(200);

    await supertest(app).get(`/polls/${createdPollId}`).expect(404);
  });
});
