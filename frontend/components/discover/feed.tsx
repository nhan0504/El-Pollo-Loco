...
export default function Feed({ pollData, setPollData }: any) {
  ...
  const [currFeed, setCurrentFeed] = useState<string | null>(
    localStorage?.getItem('feed') != null ? localStorage?.getItem('feed') : 'discover',
  );
  ...
  async function getPolls(feedType: string) {
    ...
    if (feedType === 'discover') requestType = `/feed/${page}`;
    else if (feedType === 'friends') requestType = `/feed/friends/${page}`;
    else if (feedType === 'following') requestType = `/feed/tags/${page}`;
    else {
      requestType = `/feed`;
    }
  ...
  function FeedButtons() {
    ...
    if (currFeed == 'discover') discoverStyle = styles().currFeed;
    else if (currFeed == 'friends') friendsStyle = styles().currFeed;
    else if (currFeed == 'following') followingStyle = styles().currFeed;
    else discoverStyle = styles().currFeed;
  ...
  const tagList = () => {
    ...
    if (followedTags?.length != 0 && currFeed == 'following') {
      return (
        ...
        >
          <Typography noWrap style={{}} variant="h6" sx={{ width: 'min-content', color: 'black' }}>
            You are following
          </Typography>
    ...
    } else return <Box display="none"></Box>;
  };
    ...
    for (let i = 0; i < pollData.length; i++) {
      ...
      row.push(
        <Grid item xs={4} style={{ padding: 50 }} sx={{}} key={i}>
          ...
          {PollCard({ setDataChange }, currCard, wasVotedOn(currCard.poll_id), followedTags, false)}
    ...
    // Else display a message tailored to the specific no-poll situation
      ...
      if (currFeed == 'friends') {
        if (getStorageFriends().length > 0)
          message =
            "That's the end of your friends' polls! Click on a username to add more friends.";
        else
          message =
            "You don't have any friends yet! Follow other users by clicking on their username.";
      } else if (currFeed == 'following') {
        if (followedTags.length > 0)
          message =
            "There are no more polls with your followed tags. Blue tags are ones you haven't followed yet - click to follow them.";
        else message = "You're not following any tags. Click a tag on any poll to get started.";
      } else message = "Oops! We can't get any polls right now. Try again later.";
  ...
  return isLoading ? (
    ...
    <React.Fragment>
      <Stack sx={{ width: '100%', ml: 35 }} direction="row" spacing={1}>
        ...
        <Fade in={currFeed == 'following'}>{tagList()}</Fade>
        <Fade in={currFeed == 'friends'}>{friendList()}</Fade>
      </Stack>
      <Grow in={true}>
        <Container maxWidth={false}>
          <CardsTogether />
