...
export default function MyProfile() {
  ...
  const [followedTags, setFollowedTags] = useState<string[]>(
    localStorage.getItem('tags') ? String(localStorage.getItem('tags'))?.split(',') : [],
  );
    ...
    try {
      setFollowedTags(
        localStorage.getItem('tags') != null
          ? String(localStorage.getItem('tags'))?.split(',')
          : [],
      );
  ...
  function FormRow(pollData: any) {
    ...
    for (let i = 0; i < pollData.length; i++) {
      ...
      row.push(
        <Grid item xs={6} justifyContent={'center'} alignItems="center" key={i}>
          {PollCard(
            { setDummyDataChange },
            currCard,
    ...
    if (pollData.length > 0) {
      ...
      return (
        <Typography level="body-sm">
          You haven't made any polls yet. Click the "Create Poll" button to get started!
        </Typography>
      );
    } else if (loading) {
      return (
    ...
    if (followedTags.length != 0 || loading)
            ...
            if (tag != '') {
              return (
                <Button
                  ...
                  sx={{ bgcolor: 'green', color: 'white', mx: 1, my: 0.6, maxHeight: '45%' }}
                  key={tag}
                >
                  {tag} ✓
                </Button>
      ...
      return (
        <Typography level="body-sm" style={{ textAlign: 'center' }}>
          ...
          <br />
          Click on blue poll tags to follow your favorites.
        </Typography>
      );
    }
  }
  ...
  return loading || !isAuth ? (
    ...
    <Grow in={true}>
      <Box sx={{ flex: 1, width: '100%' }}>
        ...
        >
          <Box sx={{}}>
            <Typography level="h3" sx={{ py: 1.5, px: 2 }}>
              {userData.username}'s Profile Page
            </Typography>
          </Box>
          <Stack
            direction="row"
          ...
          >
            ...
            >
              ...
              <CardOverflow sx={{ borderTop: '1px solid', borderColor: 'divider' }}></CardOverflow>

              <List sx={{ display: 'flex', flexDirection: 'column' }}>
                <ListItem disablePadding>
                  <Typography>
                    {pollData.length} {pollData.length != 1 ? 'polls' : 'poll'} created
                  </Typography>
                </ListItem>
                <ListItem disablePadding>
                  <Typography>
                    {getTotalVotes()} total {getTotalVotes() != 1 ? 'votes' : 'vote'} on their polls
                  </Typography>
                </ListItem>
                <ListItem disablePadding>
                  <Typography>
                    Voted {totalPollsVoted} {totalPollsVoted != 1 ? 'times' : 'time'}
                  </Typography>
                </ListItem>
              </List>
            </Card>
